      ...
      <div className="mx-auto grid max-w-4xl gap-4">
        {items.map((item) => (
          <details className="surface-card tool-faq-item group" key={item.question}>
            <summary className="cursor-pointer list-none text-base font-semibold text-white flex items-center justify-between gap-4">
              {item.question}
              <span className="tool-faq-chevron">▼</span>
            </summary>
            <p className="mt-4 text-base text-slate-400 leading-relaxed">{item.answer}</p>
          </details>
        ))}
      </div>
    </div>
