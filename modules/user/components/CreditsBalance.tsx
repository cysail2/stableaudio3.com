  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        aria-haspopup="menu"
        className="inline-flex h-10 items-center gap-1.5 rounded-full border border-white/10 bg-slate-900/60 px-3 text-sm font-semibold text-cyan-200 transition hover:border-cyan-300/60 hover:text-cyan-100"
        onClick={handleToggleOpen}
        title="Credit balance — view recent activity"
        type="button"
      >
        <CoinsIcon className="text-cyan-200" />
        <span>{isLoadingUserInfo ? "..." : credits}</span>
      </button>
      {isOpen ? (
        <div
          className="absolute right-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-2xl border border-white/10 bg-slate-950/95 shadow-2xl backdrop-blur-xl"
          role="menu"
        >
          <div className="flex items-start justify-between gap-3 border-b border-white/10 p-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-200">Credit balance</p>
              <p className="mt-1 text-3xl font-bold text-white">{isLoadingUserInfo ? "..." : credits}</p>
                ...
                {items.map((item) => (
                  <li className="flex items-center justify-between gap-3 text-sm" key={item.id}>
                    <div className="min-w-0">
                      <p className="truncate text-slate-200">{formatChangeType(item.change_type)}</p>
          ...
          <div className="border-t border-white/10 bg-slate-950/60 p-2">
            <Link
              className="flex items-center justify-between rounded-lg px-3 py-2 text-sm font-semibold text-cyan-200 transition hover:bg-white/5 hover:text-cyan-100"
              href="/pricing"
